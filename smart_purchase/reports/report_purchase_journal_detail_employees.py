...
class ReportPurchaseJournalDetailEmployee(models.Model):
    _name = 'report.purchase.journal.detail.employees'
    _auto = False 
    ...
    customer_code = fields.Char(string='Vendor ID')
    customer_name = fields.Char(string='Vendor name')
    ...
    def reload_data(self,*kw):
        start_date =''
        end_date = ''
        employee_list = kw[0].get('employees',None)
        if len(kw)>0:
        ...
        emp_items = tuple(employee_list.ids)
        if len(emp_items)<=1:
            emp_items = emp_items + tuple('0')+tuple('0')
        sql = """
        ...
        """ %( self._table,emp_items,start_date,end_date)

        tools.drop_view_if_exists(self._cr, '%s' % self._table) 
        self.env.cr.execute(sql)
        if 'vi_VN' in self._context.values():
            name = 'Sổ chi tiết mua hàng theo nhân viên'
        else:
            name = 'Purchase Journal Detail By Employees'
 
        return { 
            'name': name,
            "type": "ir.actions.act_window",
            "view_mode": "tree",
            "res_model": 'report.purchase.journal.detail.employees' ,
