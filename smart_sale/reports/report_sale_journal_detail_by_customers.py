...
class ReportSaleJournalDetailCustomer(models.Model):
    _name = 'report.sale.journal.detail.customers'
    _auto = False 
    ...
    def reload_data(self,*kw):
        start_date =''
        end_date = ''
        customer_list = kw[0].get('customers',None)
        if len(kw)>0:
        ...
        cust_items = tuple(customer_list.ids)
        if len(cust_items)<=1:
        ...
        if 'vi_VN' in self._context.values():
            name = 'Sổ chi tiết bán hàng theo khách hàng'
        else:
            name = 'Sale Journal Detail By Customers'
 
        return { 
            'name': name,
            "type": "ir.actions.act_window",
            "view_mode": "tree",
            "res_model": 'report.sale.journal.detail.customers' ,
